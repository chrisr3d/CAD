	...
	public Case appliquerTactique(Plateau platIA, Plateau joueur) {
		...
		if (this.toucher && this.dernierTir != null) {
			...
			if (this.dernierTir.getX() - 1 >= 0 && !trouve) {
				//On fait le 2eme test ici, pour �viter les probl�mes d'arrayOutOfBound de getCase()
			...
			//si on a pas trouv� de case sur lequel on peut tirer autour de la case bateau qu'on avait d�j� touch�
			//on r�initialise toucher et dernierTir pour ne pas a nouveau tirer autour
			if(!trouve){
				this.toucher = false;
				dernierTir = null;
			}
